...
void Person::describe(string* arr, int now, int len) {
    if (arr[now] == "parentA") this->parentA->describe(arr, now + 1, len);
    if (arr[now] == "parentB") this->parentB->describe(arr, now + 1, len);
    if (arr[now] == "mate") this->mate->describe(arr, now + 1, len);
    if (arr[now] == "child") this->child->describe(arr, now + 1, len);
    else{
        if(arr[now]=="name"){
            this->name = arr[now + 1];
        }
