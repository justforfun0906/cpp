...
node* build_tree(int *inorder, int *preorder, int inorder_start, int inorder_end, int preorder_start){
    for(int i=preorder_start;i<preorder_start+inorder_end-inorder_start+1;i++){
        for(int j=inorder_start;j<=inorder_end;j++){
            if(preorder[i] == inorder[j]){
                ...
                root->right = build_tree(inorder, preorder, j+1, inorder_end, preorder_start+j-inorder_start+1);
                return root;
            }
        }
    }
