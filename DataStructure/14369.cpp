...
Node* build(int n,int l, int r, vector<int> &pre, vector<int> &in_index, int &index){
    //important edge case
    if(l>r||!flag||index>=n) return nullptr; //base case
    Node* x = new Node(pre[index]);
    index++;
    if(l==r) return x;
    else{
        int pos = in_index[x->data];
        x->left = build(n,l, pos-1, pre, in_index, index);
        x->right = build(n,pos+1, r, pre, in_index, index);
        if(x->left){
...
void solve(){
    ...
    int root_idx = 0;
    flag = 1;
    Node* root = build(n,0, n-1, pre, in_index, root_idx);
